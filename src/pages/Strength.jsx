      ...
      <div className="relative h-7 rounded-md bg-secondary/70 overflow-hidden">
        ...
        <div
          className={cn(
            ...
            positive ? 'left-1/2 bg-emerald-500/75' : 'right-1/2 bg-red-500/70',
          )}
          style={{ width: `${width / 2}%` }}
        />
      </div>
...
export default function Strength() {
  ...
  const maxAbs = useMemo(
    () => strengths.reduce((max, row) => Math.max(max, Math.abs(row.strength)), 0),
    [strengths],
  );
        ...
        <>
          {strongest && weakest && (
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="rounded-xl border border-red-500/25 bg-red-500/10 p-3 text-right">
                ...
                <div className="text-xl font-black text-red-600 dark:text-red-400 mt-1">{weakest.currency}</div>
                <div className="text-xs font-mono">{weakest.strength.toFixed(2)}%</div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-sm font-bold">Biggest separations</h2>
            </div>
            <p className="text-[11px] text-muted-foreground mb-3">Poles apart = stronger movement potential, not a direction signal.</p>
            <div className="space-y-2">
              {separations.slice(0, 8).map((row, index) => (
                <div key={row.pair} className="flex items-center justify-between rounded-lg bg-secondary/50 px-3 py-2.5">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-muted-foreground w-4">{index + 1}</span>
                    <span className="text-sm font-bold">{row.pair}</span>
                  </div>
                  <span className="text-xs font-mono font-bold text-primary">{row.separation.toFixed(2)}%</span>
                </div>
              ))}
            </div>
          </div>
