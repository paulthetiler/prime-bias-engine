...
function getFavouriteInstruments() {
  try {
    const raw = JSON.parse(localStorage.getItem('primebias_favourite_instruments') || '[]');
    if (!Array.isArray(raw)) return [];
    return raw.filter(asset => ASSETS.includes(asset));
  } catch {
    return [];
  }
}

function saveFavouriteInstruments(favourites) {
  localStorage.setItem('primebias_favourite_instruments', JSON.stringify(favourites));
}
...
export default function Input() {
  ...
  const [favouriteInstruments, setFavouriteInstruments] = useState(() => getFavouriteInstruments());
  ...
  const orderedAssets = [
    ...
    ...ASSETS.filter(asset => !favouriteInstruments.includes(asset)),
  ];

  const toggleFavouriteInstrument = (asset) => {
    setFavouriteInstruments(prev => {
      const next = prev.includes(asset)
        ? prev.filter(item => item !== asset)
        : [...prev, asset];
      saveFavouriteInstruments(next);
      return next;
    });
  };
  ...
  return (
    <div className="p-3 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2 rounded-lg border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
          <div className="flex items-start gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {autoSaveError && <span className="block text-[11px] opacity-80 break-words">{autoSaveError}</span>}
            </div>
          </div>
          <Button variant="outline" size="sm" onClick={retryAutoSave} className="h-7 gap-1.5 shrink-0 border-destructive/40 text-destructive hover:text-destructive">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This clears {instrument} and its indicator inputs from the Bias Tool and Summary on this device. Your completed trades, journals and history are not affected. If it is starred as a favourite, it will stay in your favourites list.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="space-y-2">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="h-12 w-full justify-between text-base font-normal">
              ...
              <CommandList>
                <CommandGroup>
                  {orderedAssets.map((asset, index) => {
                    const isFavourite = favouriteInstruments.includes(asset);
                    const showDivider = favouriteInstruments.length > 0 && index === favouriteInstruments.length;
                    return (
                      <React.Fragment key={asset}>
                        {showDivider && <div className="mx-2 my-1 border-t border-border" aria-hidden="true" />}
                        <CommandItem
                          value={asset}
                          onSelect={val => {
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              toggleFavouriteInstrument(asset);
                            }}
                            className="ml-2 h-9 w-9 inline-flex items-center justify-center rounded-md text-muted-foreground hover:text-amber-500 hover:bg-secondary"
                          >
                            <Star className={cn('h-4 w-4', isFavourite && 'fill-current text-amber-500')} />
                          </button>
                        </CommandItem>
                      </React.Fragment>
                    );
      ...
      {instrument && (
        <>
          <div className="space-y-1">
            {TIMEFRAMES.map(tf => (
              <TimeframeRow key={tf.key} timeframe={tf} indicators={inputs[tf.key]} onChange={indicators => handleTFChange(tf.key, indicators)} />
            ))}
          </div>
          <ExtraCheck value={extraCheck} onChange={handleExtraCheckChange} />
          <BiasResult results={results} />
          {baseAtr != null && (
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg border border-border bg-card p-3 text-center">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">ATR Used</div>
                <div className="font-mono text-sm font-bold mt-1">{formatWithUnit(baseAtr, instrument)}</div>
              </div>
              <div className="rounded-lg border border-border bg-card p-3 text-center">
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Minimum Safe Move</div>
                <div className="font-mono text-sm font-bold mt-1">{targetInfo == null ? '—' : formatWithUnit(targetInfo, instrument)}</div>
              </div>
            </div>
          )}
        </>
      ...
      <div className="pt-2 flex justify-end">
        <Button variant="ghost" size="sm" onClick={() => navigate('/dashboard')}>Back to Summary</Button>
      </div>
    </div>
  );
}
